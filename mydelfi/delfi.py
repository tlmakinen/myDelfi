...
class Delfi():
    ...
    def __init__(self, data, prior, nde, \
                 ...
                 restore_filename = "restore.pkl", restore = False, save = True):
    ...
    def log_likelihood_individual(self, i, theta, data):
        ...
        lnL = self.nde[i].eval((np.atleast_2d((theta-self.p_mean)/self.p_std), np.atleast_2d((data-self.x_mean)/self.x_std)))
    ...
    def log_likelihood_stacked(self, theta, data):
        ...
        for n in range(self.n_ndes):
            L += self.stacking_weights[n]*np.exp(self.nde[n].eval((np.atleast_2d((theta-self.p_mean)/self.p_std), np.atleast_2d((data-self.x_mean)/self.x_std))))
        lnL = np.log(L)
